...
- **CSR encoding**: base64 of the raw DER PKCS#10 bytes. ⚠️ **Flag to verify against the real upstream during this epic**: the confirming answer's exact wording was *"yes that's correct, raw DER PKCS#10 bytes encoded in base64 with the corresponding --- BEGIN... headers"* — the first half confirms raw-DER-then-base64 (no PEM), but the trailing clause about `-----BEGIN...` headers is contradictory for that interpretation (raw DER has no such headers; only a PEM-formatted text blob would). Implement to the raw-DER-base64 interpretation (matches the JSON contract as originally specified: `{"CSR": "<base64 csr string>"}`, and matches RFC 7030's own CSR wire encoding), but treat the header-encoding as a one-line confirmation to get from the user (or from a real request against the actual upstream API) before this epic is marked fully done — do not let it block building and testing against the documented contract, since the WireMock-based test suite pins whichever interpretation is implemented and a later correction is a small, contained change to one encoding call site.
