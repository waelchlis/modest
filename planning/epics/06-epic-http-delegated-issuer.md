...
- **CSR encoding**: base64 of the **PEM-encoded** PKCS#10 request, not base64 of the raw DER. ✅ **Resolved 2026-08-25** (was flagged here as contradictory — see [../09-open-questions.md](../09-open-questions.md) #1 for the full history): reconfirmed directly against the real upstream. `HttpDelegateIssuer.IssueAsync` builds the field as `base64(PemEncoding.WriteString("CERTIFICATE REQUEST", der))`.
